    ...
    switch (CurrentState) {
            ...
            switch (ThisEvent.EventType) {
                ...
                case ES_TIMEOUT:
                    if (ThisEvent.EventParam == 9) {
                        variablePercentRightTurn(CIRCLE_BEACON_PERCENTAGE, MEDIUM_MOTOR_SPEED * 1.15);
        ...
        case GoAroundFirstBeaconOtherWay:
            switch (ThisEvent.EventType) {
                ...
                case ES_TIMEOUT:
                    if (ThisEvent.EventParam == 9) {
                        variablePercentLeftTurn(CIRCLE_BEACON_PERCENTAGE, MEDIUM_MOTOR_SPEED * 1.15);
                        ES_Timer_StopTimer(3);
        ...
        case StartCentering:
            switch (ThisEvent.EventType) {
                ...
                case BEACON_LOST:
                    ...
                    //                    makeTransition = TRUE;
                    variablePercentRightTurn(60, MEDIUM_MOTOR_SPEED);
                    ThisEvent.EventType = ES_NO_EVENT;
                    break;
                case BEACON_DETECTED:
                    variablePercentLeftTurn(60, MEDIUM_MOTOR_SPEED);
                    ThisEvent.EventType = ES_NO_EVENT;
                    break;
                case TAPE_ON:
                    //                    nextState = RepositionOffTape;
        ...
        case RepositionOffTape:
            switch (ThisEvent.EventType) {
                case ES_ENTRY:
                    ...
                    //                    driveBackward(SLOW_MOTOR_SPEED * 0.7);
        ...
        case FindingTapeAfterSecondBeacon:
            switch (ThisEvent.EventType) {
                ...
                case ES_TIMEOUT:
                    if (ThisEvent.EventParam == 1) {
                        rightTankTurn(MEDIUM_MOTOR_SPEED);
                        ES_Timer_InitTimer(2, 75 * MS_PER_DEGREE_TURN_MED_SPEED);
                    } else if (ThisEvent.EventParam == 2) {
                        driveForward(MEDIUM_MOTOR_SPEED);
                    }
                    ThisEvent.EventType = ES_NO_EVENT;
